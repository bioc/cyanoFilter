...
celldebris_nc <- function (flowframe, channel1 = "RED.B.HLin", channel2 = "YEL.B.HLin",
                            interest = c("BS4", "BS5", "Both"), to_retain = c("refined", "potential")) {
  ...
  ###Full flowframe
  #Forming a new expression matrix for the full flowframe with indicator added for BS4 or BS5
  nexp_mat <- as.matrix(cbind(flowCore::exprs(flowframe),
  ...
  return(list(fullframe = fflowframe, reducedframe = NA, Cell_count = n_cyano,
