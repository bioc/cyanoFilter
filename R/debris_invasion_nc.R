...
debris_inc <- function(flowframe, p1, p2) {
  ...
  flowDensity::plotDens(flowframe, c(p1, p2), 
                        main = flowCore::identifier(flowframe), 
                        frame.plot = F)
  #debris gating
  ...
  if(length(p1_peaks$Peaks) == 2) {
    ...
    if(p1_peaks$Peaks[1] - (1 * sd(flowframe@exprs[, p1])) <= 0  ) {
      ...
      deb_cut <- ifelse(length(flowDensity::getPeaks(flowframe2, p1, tinypeak.removal = 1/5)$Peaks) < 2, 
                        flowDensity::deGate(flowframe, p1, use.upper = T, upper = F), 
                        deb_cut)
    ...
    } else if(p1_peaks$Peaks[1] - (2 * sd(flowframe@exprs[, p1])) > 0 & 
              deb_cut >= p1_peaks$Peaks[1]) {
    ...
    deb_cut <- p1_peaks$Peaks[2] - (0.5 * sd(flowframe@exprs[, p1]))
  }
