...
#' @param control a logical vector indicating if a control flowframe will be supplied. Defaults to T
...
#' @param day1mono_control is the control flowframe on first day if monocultures are being gated
#' @param day1b4_control control flowframe for BS4 if a mix of BS4 and BS5 are being gated.
#'                       Defaults to NULL
#' @param day1b5_control control flowframe for BS5 if a mix of BS4 and BS5 are being gated.
#'                       Defaults to NULL
#' @return rframe reduced flowframe after debris have been filtered out.
#' @return fullframe full flowframe with indicator for debris in the expression matrix
#' @return Cell_count total number of cyano cells counted. This will be the number of BS4
#'                    if interest is BS4 and number of BS5 if interest is BS5 and addition of both
#'                    if interest is both.
#' @return N_particle is the total number of particles counted.
celldebris <- function (flowframe, p1 = "RED.B.HLin", p2 = "YEL.B.HLin", positions = c(F, NA),
  ...
  if(interest == "Both"){
    ...
    day1 <-  flowDensity::flowDensity(day1mono_control,
                         channels = c("RED.B.HLin", "YEL.B.HLin"),
                         position = c(F, NA), upper=c(T, T),
                         use.upper = c(F, T))
    day1_2 <- day1mono_control[which(is.na(flowCore::exprs(day1@flow.frame)[, 1])), ]
  }
  ...
  if(control==F | is.null(control_flowFrame)){
    ...
    if(interest == "BS4"){
      ...
      bs4 <- flowDensity::flowDensity(bs4s, channels = c(p1,p2), position = c(NA,F),
      ...
      BS4_exx <- as.matrix(cbind(flowCore::exprs(flowframe)[which(!is.na(BS4BS5_ind) & BS4BS5_ind != 1), ],
                                 as.numeric(BS4_ind)))
      #1=BS4, 0=debris
      ...
      if(stringr::str_detect(msg, "peak") == T |
         modes::bimodality_coefficient(flowCore::exprs(bs4bs5)[,"YEL.B.HLin"])<=0.6){
        bs5 <- flowDensity::flowDensity(bs4bs5, channels = c(p1, p2), position = c(T, T),
                           use.percentile = c(T, T),
      ...
      #reduced flow frame for BS 5
      bs5_reduced <- flowDensity::getflowFrame(bs5)
      n_BS5 <- flowCore::nrow(flowCore::exprs(bs5_reduced))
      #plotting
      #BS5
      ...
      BS5_exx <- as.matrix(cbind(flowCore::exprs(flowframe)[which(!is.na(BS4BS5_ind) & BS4BS5_ind!=2),],
                                 as.numeric(BS5_ind)))
      #1=BS5, 0=debris
      colnames(BS5_exx)[length(colnames(BS5_exx))] <- 'Debris.Indicator'
      BS5_fflowframe <- new("flowFrame", exprs = BS5_exx,
    ...
    if(interest == "BS4"){
      ...
      #reduced flow frame for BS4
      bs4_reduced <- flowDensity::getflowFrame(bs4)
      n_BS4 <- flowCore::nrow(flowCore::exprs(bs4_reduced))
      #plotting
      points(bs4@filter, type = "l", col = 2, lwd = 2, lty = 4)
      text(mean(bs4@filter[,1]), mean(bs4@filter[,2]), "BS4", col=2)
      ...
      msg <- stringr::str_sub(capture.output(flowDensity(bs4bs5,channels = c(p1, p2),
                                                position = c(NA,F)))[1], 6, 27)
      #BS5
      if(stringr::str_detect(msg,"peak")==T |
         modes::bimodality_coefficient(flowCore::exprs(bs4bs5)[,"YEL.B.HLin"]) <= 0.6){
