...
phyto_filter <- function(flowfile, pig_channels = NULL, com_channels = NULL,
                         ph = 0.05, proportion = 0.80) {

  if(is.null(pig_channels) & is.null(com_channels)) {

    stop("both pigment and cell complecity channels cannot be NULL. Check your input")
  ...
  if(sum(is.na(gen_togate)) == length(gen_togate)) {
        ...
        full_flowframe <- new_flowframe(flowfile, group = rep(1, nrow(flowfile)), togate = NULL)
        plott1 <- ggpairsDens(full_flowframe, channels = channels, group = "Clusters")
        message("Only one cluster found across the supplied channels.")

        return(list(full_flowframe = full_flowframe, plot = plott1))

    } else {

      clusters <- fgate$full_flowframe@exprs[, paste(gen_togate[!is.na(gen_togate)], "Cluster", sep = "_")]
      if(is.null(dim(clusters))) {
        ...
        is <- apply(clusters, 1, function(x) {
          ...
          ii <- c()
          for(i in 1:nrow(cluster_unique)) {

            if(sum(x == cluster_unique[i, ]) == ncol(cluster_unique)) ii <- c(ii, i) else next
          }
      ...
      full_flowframe <- new_flowframe(fgate$full_flowframe, group = is, togate = NULL)
      needed_proportion <- cluster_extractp(full_flowframe, cluster_var = "Clusters",
      ...
      plott1 <- ggpairsDens(flowframe_proportion, channels = channels, group = "Clusters")
