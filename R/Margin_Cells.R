...
#'             supplies a cut off point on the channel
...
cellmargin <- function(flow.frame, Channel = "SSC.W", type = c("manual", "estimate"), cut = NULL){
  if(type == "manual" & !is.null(cut)){
    ...
    infl <- c(FALSE, diff(diff(bfilter$y)) > 0)
    ...
    margin.ind <- ifelse(flowCore::exprs(flow.frame)[, Channel] <= infl_point,T,F) #FALSE=Margin Event
