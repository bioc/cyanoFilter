...
cellmargin <- function(flow.frame, Channel = "SSC.W", type = c("manual", "estimate"), cut = NULL) {
    if (type == "manual" & !is.null(cut)) {
        ...
        # bfilter <- density(exprs(flow.frame)[,Channel]) infl <- c(FALSE, diff(diff(bfilter$y) > 0) > 0) if(length(unique(infl)) > 1){ infl_point <-
        # max(bfilter$x[infl],na.rm=T) }else{ infl_point <- max(bfilter$x,na.rm=T) }
        ...
        margin.ind <- ifelse(flowCore::exprs(flow.frame)[, Channel] > infl_point2 & flowCore::exprs(flow.frame)[, Channel] < infl_point1, T, F)  #FALSE=Margin Event
    ...
    return(list(reducedflowframe = rflowframe, fullflowframe = fflowframe, N_margin = n_margin, N_cell = nrow(rflowframe), N_particle = nrow(fflowframe)))
}
