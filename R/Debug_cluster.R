...
#finite <- flowCore::rectangleGate(filterId="positive", .gate=mat)

#cols <- cols.all[c(1,4,5,7,8)]

# log transform all the data
#logtrans <- flowCore::transformList(cols, logTransform()) #log transform all parameters

#fil <- 5
#dataset <- 10
#if (fil>15){ cyano.ranges <- matrix(c(1.8,2.8,0.0,1.5,1.5,2.5,1.5,2.5,1.2,2.5,
#                                             1.8,3.0,1.5,3.0,1.5,3.0,1.5,2.5,1.0,2.5),2,10)}
#data.set <- flowCore::read.FCS(org.files[fil], emptyValue = F, dataset = samples[[fil]][dataset],
#                     alter.names = T)
