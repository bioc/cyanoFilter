...
\author{Oluwafemi D. Olusoji\\Universiteit Hasselt \And
Frederik De Laender\\Universite De Namur \And
Jurg Spaak\\Universite De Namur
\AND Thomas Neyens\\Universiteit Hasselt \And
...
\title{cyanoFilter: An \proglang{R} Package for Identifying Synechococcus Population Obtained via Flow Cytometry}
\Plaintitle{An R Package for Identifying Synechococcus Population Obtained via Flow Cytometry}
\Shorttitle{cyanoFilter: Identifying Synechococcus Population Using \proglang{R}}
...
This tutorial demonstrates a set of reproducible steps to follow to filter out synechococcus population from data obtained via flow cytometry using the \proglang{cyanoFilter} package in \proglang{R}. The steps also involve data quality checks since these experiments are typically performed at different dilution levels. Furthermore, \proglang{cyanoFilter} can return full expression matrices with indicators for each identified cell type. The steps are demonstrated using resulting data from an experiment involving synechoccus bacteria under some environmental stressor.

\pkg{cyanoFilter} provides a reproducible approach to identifying and extracting synechococcus population as well as other flow cytometry outcomes (debris, doublets and margin events). It also opens the door to further research where full effect of environmental stressors on synechococcus can be studied.
...
\Keywords{flow cytometry, synechococcus, ecology,reproducible research, \proglang{R}}
\Plainkeywords{flow cytometry, synechococcus, ecology, reproducible research, R}
...
\Address{
  ...
  E-mail: \email{oluwafemi.olusoji@uhasselt.be}\\
  \emph{and}\\
  ...
  Universite De Namur\\
  5020, Namur, Belgium.\\
  E-mail: \email{oluwafemi.olusoji@unamur.be}\\
  Jurg Spaak\\
  Evolutionary Biology Unit\\
  Universite De Namur\\
  5020, Namur, Belgium.
  E-mail: \email{jurg.spaak@uhasselt.be}\\
  Thomas Neyens\\
  Center for Statistics\\
  Universiteit Hasselt\\
  3590, Diepenbeek, Belgium.\\
  E-mail: \email{thomas.neyens@uhasselt.be}\\
  Marc Aerts\\
  Center for Statistics\\
  Universiteit Hasselt\\
  3590, Diepenbeek, Belgium.\\
  E-mail: \email{marc.aerts@uhasselt.be}\\
}
...
Flow cytometry and its outcomes
Knowing fully well that outcomes from a flow cytometry experiment can be; debris (dead particles), doublets (cells with disproportionate width-height relationship), margin events (cells too large to measure) and singlets (good cells), these set of steps are designed to identify each possible outcome and return the desired cell population. It is largely applied in biomedical and medical sciences but there are recent applications of the technique in Ecology.



Synechococcus cyano bacteria and its ecological importance
They bacteria are also one of the known oldest life forms known to obtain their energy via photosynthesis.



The central aim of this tutorial is to introduce readers to the software package \pkg{cyanoFilter}. The software package aims at identifying populations of two \emph{synechococcus} type cyanobacteria obtained via flow cytometry expriments. It also provides ecologist working on these \emph{synechococcus} type cyanobacteria with a toolset that not only helps them identify them but also helps them with indicators for other potential outcomes, which paves way for joint analysis of these populations. The package also reveals that reproducibility is quite possible in experiments whose outcome is largely driven by experts knowledge.

This tutorial is structured as follows; a review of existing methods for identifying cell populations in flow cytomtery experiments alongside the choice of method employed in \pkg{cyanoFilter} is presented in section 2. Section 3 introduces the reader to some essential and crucial properties of the synechococcus population to be identified. The next section presents the \pkg{cyanoFilter} framework, its functions alongside their purpose. Section 5 presents the \emph{Costa} experiment, whose results will be used to demonstrate the functionalities of \pkg{cyanoFilter}, the last section gives illustrative examples on use of the package.

\section[methods]{Exisitng methods for identifying cell populations in flow cytometry}\label{sec:methods}
Manual identification of cell population by experts via a two dimensional dotplot or one dimensional histogram is the most common method of identifying flow cytometry outcomes. This process is largely subjective and makes reproducibility difficult(\cite{ONeill:2013}). Hence, there has been the need to develop software tools to identify cell populations present in flow cytometry experiments. As of present, there are a number of tools available, we review some of them in the paragraph below.

Available tools for identifying flow cytometry outcomes are based on different approaches; \cite{Aghaeepour:2011} developed the \pkg{flowMeans} package based on \emph{k-means} and a change-point algorithm to identfy populations present in a flow cytometry data. The package also merges local cluster using the Mahalanobis distance metric. The package can identify concave cell populations, however it is limited when it comes to cell populations with irregular shapes (\cite{Ge:2012}). \cite{Ge:2012} developed the \pkg{flowPeaks} package which is based on unsupervised \emph{k-means} and \emph{finite mixture modelling} to identify cell populations in flow cytometry data.

, however these methods focus more on experiments involving human cells.

\section[synechococcus]{Crucial Synechococcus Properties}\label{sec:methods}


\section[software]{Software}\label{sec:software}

\section[CostaExperiment]{The Costa Experiment}\label{sec:dataset}
